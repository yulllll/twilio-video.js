...
var ParticipantSignaling = function (_StateMachine) {
  ...
  _createClass(ParticipantSignaling, [{
    ...
    value: function updateSubscriberTrackPriority(trackSid, priority, renderHint) {
      // note the most recent priority update for the track.
      this._enqueuedPriorityUpdates.set(trackSid, { priority: priority, renderHint: renderHint });
    ...
    value: function setTrackPrioritySignaling(trackPrioritySignaling) {
      ...
      if (trackPrioritySignaling) {
        this._enqueuedPriorityUpdates.forEach(function (_ref, trackSid) {
          var priority = _ref.priority,
              renderHint = _ref.renderHint;

          _this2._trackPrioritySignaling.sendTrackPriorityUpdate(trackSid, 'subscribe', priority, renderHint);
        });
