...
describe('Room', function() {
  ...
  (defaults.topology !== 'peer-to-peer' ? describe.only : describe.skip)('"trackSwitched" events', () => {
    let alice;
    let bob;
    let remoteBob;
    let charlie;
    ...
    it('"trackSwitchedOff" fires on RemoteTrack ("switchedOff") RemoteTrackPublication, RemoteParticipant and Room', async () => {
      ...
      const charlieTrackPubPromise = charlie.publishTrack(hiPriTrack, { priority: PRIORITY_HIGH });
      const charlieTrackPub = await waitFor(charlieTrackPubPromise, `${charlie.sid} to publish a high priority LocalTrack: ${aliceRoom.sid}`);
