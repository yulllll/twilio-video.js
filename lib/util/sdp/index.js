...
function getOenH264PT(mediaSection) {
  // look for entry containing: level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=42e01f
  // something like: a=fmtp:125 level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=42e01f
  // and return 125;
  return getPayloadTypesInMediaSection(mediaSection).find(pt => {
    const matchPattern = new RegExp(`a=fmtp:${pt} level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=42e01f`);
    const match = mediaSection.match(matchPattern);
    return match;
  });
}
...
function setCodecPreferences(sdp, preferredAudioCodecs, preferredVideoCodecs) {
  ...
  return [session].concat(mediaSections.map(section => {
    ...
    if (window.openH264 && kind === 'video') {
      // put PT associated with following line first.
      // a=fmtp:125 level-asymmetry-allowed=1;packetization-mode=1;profile-level-id=42e01f
      console.log('codecMap: ', codecMap);
      console.log('payloadTypes: ', payloadTypes);

      const specialPT = getOenH264PT(section);
      console.log('specialPT:', specialPT);

      // move specialPT to front.
      if (payloadTypes.indexOf(specialPT) > 0) {
        payloadTypes.splice(payloadTypes.indexOf(specialPT), 1);
        payloadTypes.unshift(specialPT);
        console.log('updated payloadTypes: ', payloadTypes);
      }
    }
