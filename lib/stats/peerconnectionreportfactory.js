...
class PeerConnectionReportFactory {
   ...
   */
  next() {
    // maktodo: firefox/chrome is handled here.
    const updatePromise = guessBrowser() === 'firefox'
      ? updateFirefox(this)
      : updateChrome(this);

    return updatePromise.then(() => {
      console.log('maktodo: updatePromise');
      const audioSenderReportFactories = Array.from(this.audio.send.values());
      const videoSenderReportFactories = Array.from(this.video.send.values());
      const audioReceiverReportFactories = Array.from(this.audio.recv.values());
      const videoReceiverReportFactories = Array.from(this.video.recv.values());
...
function updateFirefox(factory) {
  console.log('maktodo: reports are queried here.');
...
function updateChrome(factory) {
  console.log('maktodo: chrome reports are queried here.');
