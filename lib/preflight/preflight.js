...
const PreflightTestLite = require('./preflightlite');
...
//
// VMS implementation of network quality score is here:
// https://code.hq.twilio.com/twilio/vms-media-server/blob/master/kms-sfu/src/server/implementation/NetworkQualityAlgorithm.cpp
function runPreflight(token, options) {
  ...
  if (typeof token !== 'string') {
    // eslint-disable-next-line new-cap
    throw E.INVALID_TYPE('token', 'string');
  }

  return new PreflightTestLite(token, options);
}
