...
function runPreflightTest(token, options, preflightTest) {
  ...
  function collectMOSDataForTrack(srcTrackStatsArray, targetTrackStats, name) {
    if (srcTrackStatsArray && srcTrackStatsArray[0]) {
      const srcTrackStats = srcTrackStatsArray[0];
  ...
  function collectMOSData(subscriberStats, publisherStats, collectedStats) {
    const { localAudioTrackStats,  localVideoTrackStats } = publisherStats;
    const { remoteAudioTrackStats, remoteVideoTrackStats } = subscriberStats;

    collectMOSDataForTrack(localAudioTrackStats, collectedStats.localAudio, 'localAudio');
    collectMOSDataForTrack(localVideoTrackStats, collectedStats.localVideo, 'localVideo');

    collectMOSDataForTrack(remoteAudioTrackStats, collectedStats.remoteAudio, 'remoteAudio');
    collectMOSDataForTrack(remoteVideoTrackStats, collectedStats.remoteVideo, 'remoteVideo');
  ...
  function collectRTCStats(collectedStats) {
    return Promise.all([bobPC, alicePC].map(pc => getStatsForPC(pc)))
      ...
      .then(([subscriberStats, publisherStats]) => {
        {
          collectMOSData(subscriberStats, publisherStats, collectedStats);
  ...
  function generatePreflightReport(collectedStats) {
    ...
    return {
      ...
      stats: {
        ...
        remoteVideo: {
          mos: makeStat(collectedStats.remoteVideo.mos),
          jitter: makeStat(collectedStats.remoteVideo.jitter),
          rtt: makeStat(collectedStats.remoteVideo.rtt),
          packetLoss: makeStat(collectedStats.remoteVideo.packetLoss)
        },
        remoteAudio: {
          mos: makeStat(collectedStats.remoteAudio.mos),
          jitter: makeStat(collectedStats.remoteAudio.jitter),
          rtt: makeStat(collectedStats.remoteAudio.rtt),
          packetLoss: makeStat(collectedStats.remoteAudio.packetLoss)
        },
