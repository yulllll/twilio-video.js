 ...
 */
  ...
  function collectStatsForDuration(duration) {
    return collectRTCStatsForDuration(duration);
  }
  ...
  function realTest(iceServers, audioTrack, videoTrack) {
    return new Promise((resolve, reject) => {
      ...
      alicePC = new DefaultRTCPeerConnection({ iceServers, iceTransportPolicy });
      bobPC = new DefaultRTCPeerConnection({ iceServers, iceTransportPolicy });
      ...
      alicePC.createOffer().then(offer => {
        return Promise.all([
          alicePC.setLocalDescription(offer),
          bobPC.setRemoteDescription(offer)
        ]);
      }).then(() => {
        return bobPC.createAnswer();
      }).then(answer => {
        return Promise.all([
          bobPC.setLocalDescription(answer),
          alicePC.setRemoteDescription(answer)
        ]);
      }).catch(error => {
        reject(error);
      });
