    ...
    switch (transport.state) {
      case 'connected':
        switch (message.type) {
          case 'connected':
          case 'synced':
            transport._base_document = message
          case 'update':
            const patch = compare(transport._base_document, message)
            const baseLength = JSON.stringify(transport._base_document).length
            const patchLength = JSON.stringify(patch).length
            const messageLength = JSON.stringify(message).length
            console.log("%: ",
                Math.floor( (messageLength - patchLength) / messageLength * 100),
                "Savings: ", messageLength - patchLength, " Base: ", baseLength,
