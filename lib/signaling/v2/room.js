  ...
  const interval = setInterval(() => {
    roomV2.getStats().then(stats => {
      ...
      stats.forEach((response, id) => {
        ...
        const report = new StatsReport(id, response, true /* prepareForInsights */, roomV2.localParticipant.networkQualityLevel || 0);
        const validMosValues = [].concat(report.localAudioTrackStats).concat(report.localVideoTrackStats).filter(stat => stat.mosValue).map(stat => stat.mosValue);
        if (validMosValues.length) {
          const sumValues = validMosValues.reduce((previous, current) => previous + current, 0);
          const averageMosValue = sumValues / validMosValues.length;
          // eslint-disable-next-line no-console
          const score = mos.mosToScore(averageMosValue);
          roomV2.setMosScore(score);
        } else {
          roomV2.setMosScore(0);
        }
